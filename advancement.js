"use strict";
const DEBUG = true;
function choiceRuleToString(choiceRule) {
...
function basicAdvancementActionToString(action) {
    if ("skill" in action) {
...
function fillAdvancementActionBlock(name, advancementActions) {
    const block = advancementActionTemplate.cloneNode(true);
    block.querySelector(".name").innerHTML = name;
    return block;
}
function onInputChanged(event, input, choiceRule) {
    console.log("input changed", input);
    //console.log(input.closest("ul")!.querySelectorAll(":scope > li > input"));
    let checkedCount = 0;
    input.closest("ul").querySelectorAll(":scope > li > label > input").forEach(i => {
        if (i.checked) {
            checkedCount++;
        }
    });
    ...
    if ("howMany" in choiceRule && choiceRule.howMany !== 1) {
        if (checkedCount >= choiceRule.howMany) {
            ...
            input.closest("ul").querySelectorAll(":scope > li > label > input").forEach(i => { if (i.checked == false)
                i.disabled = true; });
        }
        else {
            console.log("do not prevent");
            input.closest("ul").classList.remove("locked");
            input.closest("ul").querySelectorAll(":scope > li > label > input").forEach(i => i.disabled = false);
        }
    }
    return true;
}
function transformInput(input, name, index, choiceRule) {
...
function fillSuperFeatureBlock(parentName, name, state, choiceRule, choices) {
    if (DEBUG)
        console.group("parent", parentName, "this", name);
    const block = superFeatureTemplate.cloneNode(true);
    block.querySelector(".name").innerHTML = name;
    ...
    for (const choice of choices) {
        if ("choices" in choice) {
            // SuperFeature
            if (DEBUG)
                console.log("SuperFeature", choice.name);
            const elem = fillSuperFeatureBlock(name, choice.name, choice.state, choice.choiceRule, choice.choices);
            transformInput(elem.querySelector("input"), name.replace(" ", "_"), i, choiceRule);
            ...
            if ("name" in choice) {
                ...
                if (DEBUG)
                    console.log("GroupedAdvancementAction", choice.name);
                ...
                if (DEBUG)
                    console.log("GroupedAdvancementAction", "noname");
                const feature = basicAdvancementActionTemplate.cloneNode(true);
                transformInput(feature.querySelector("input"), parentName.replace(" ", "_"), i, choiceRule);
